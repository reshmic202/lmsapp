...
const SignUp = () => {
    ...
    const handleSignup=async()=>{
      ...
      try{
        ...
        let res:any=await fetch(`${process.env.NEXT_PUBLIC_API_KEY}/user/create-user`,{
            method: 'POST',
            headers: {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gradient-to-r lg:px-32 md:px-8 px-4  from-indigo-200 via-purple-200 to-purple-300">
            <div className=' rounded-2xl w-full md:w-1/2 lg:w-1/3 p-4 bg-white py-10'>
                <h1 className=' text-2xl text-center mb-4 font-bold'>Register an account</h1>
                <div className=' flex items-center gap-1 rounded-xl px-2 bg-slate-100'>
                    ...
                    <input required name='name' onChange={handleChange} className='w-full p-2 rounded-md bg-slate-100  focus:outline-none text-sm' type="text" placeholder='Name' />
                </div>
                <div className=' flex items-center gap-1 mt-2 rounded-xl px-2 bg-slate-100'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                    </svg>
                ...
                <div>
                    <button onClick={handleSignup} className='w-full mt-2 px-4 py-2 text-white bg-slate-600 rounded-xl hover:bg-slate-700'>Signup</button>
                </div>
                <div className=' flex items-center justify-center mt-1'>
                    <Link href={"/login"}  className=' text-slate-500 font-semibold mt-2 cursor-pointer'>Already have an account? Login</Link>
                </div>
            </div>
            {toast && (
